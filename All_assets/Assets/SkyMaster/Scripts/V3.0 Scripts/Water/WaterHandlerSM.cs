...
public class WaterHandlerSM : MonoBehaviour {
		...
		public enum WaterPreset {Custom, Emerald, Muddy, Caribbean, River, Ocean, DarkOcean, FocusOcean, SmallWaves, Lake, Atoll, Mobile};
			...
			if (cam_transf.position.y < this_transf.position.y && !DisableUnderwater) {
				...
				//if(prevWaterType != waterType){
					...
					if(waterType == WaterPreset.Custom){
						...
						oceanMat.SetVector("_GAmplitude", Vector4.Lerp(Fades2,new Vector4(0.18f,0.83f,0.06f,0.222f) + WaveAmpOffset,110.27f * Time_delta * 2));				//
						...
						oceanMat.SetVector("_InvFadeParemeter", Vector4.Lerp(Fades,new Vector4(0.01f + ShoreBlendOffset,0.04f,0.043f,0.7f),110.27f * Time_delta * 2));

						oceanMat.SetVector("_BumpTiling", Vector4.Lerp(Fades1,new Vector4(1f+bumpTilingXoffset,1f + bumpTilingYoffset,0.08f,0.16f),110.27f * Time_delta * 2));
						oceanMat.SetFloat ("_GerstnerIntensity", Mathf.Lerp (oceanMat.GetFloat ("_GerstnerIntensity"), 18.16f, 0.15f *Time_delta * Sm_speed));
						oceanMat.SetVector("_GAmplitude", Vector4.Lerp(Fades2,new Vector4(1f,1.8f,0.0f,0.36f) + WaveAmpOffset,110.27f * Time_delta * 2));
						oceanMat.SetVector("_GFrequency", Vector4.Lerp(Fades3,new Vector4(0.27f,0.74f,0f,0.75f) + WaveFreqOffset,110.27f * Time_delta * 2));
						oceanMat.SetVector("_GSteepness", Vector4.Lerp(Fades4,new Vector4(6.96f,2f,5.92f,2f),110.27f * Time_delta * 2));
						oceanMat.SetVector("_GSpeed", Vector4.Lerp(Fades5,new Vector4(-2.53f,2f,1f,3f) + WaveSpeedOffset,110.27f * Time_delta * 2));
						oceanMat.SetVector("_GDirectionAB", Vector4.Lerp(Fades6,new Vector4(0.1f,-0.41f,-0.2f,0.1f) + WaveDir1Offset,110.27f * Time_delta * 2));
						oceanMat.SetVector("_GDirectionCD", Vector4.Lerp(Fades7,new Vector4(0.52f,-0.6799f,0.7175f,-0.2f) + WaveDir2Offset,110.27f * Time_delta * 2));								
				...
				if (TerrainManager != null) {
					if (Camera.main != null) {
						...
						if ( (((cam_transf.position.y < this_transf.position.y) & (!UnderOnWave | (UnderOnWave & heightController == null))) | ( UnderOnWave & heightController != null & (cam_transf.position.y < wave_height.y*1 | cam_transf.position.y < this_transf.position.y)) ) & !DisableUnderwater) {
							...
							//v3.2
							if (UnderOnWave) {
								RenderSettings.fogDensity = Mathf.Lerp(RenderSettings.fogDensity, 0.05f, Time.deltaTime*0.1f);
