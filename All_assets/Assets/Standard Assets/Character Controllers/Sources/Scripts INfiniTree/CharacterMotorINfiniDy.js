...
class CharacterMotorMovementINfiniDy {
...
var movement : CharacterMotorMovementINfiniDy = CharacterMotorMovementINfiniDy();

enum MovementTransferOnJumpINfiniDy {
...
class CharacterMotorJumpingINfiniDy {
...
var jumping : CharacterMotorJumpingINfiniDy = CharacterMotorJumpingINfiniDy();

class CharacterMotorMovingPlatformINfiniDy {
	var enabled : boolean = true;
	
	var movementTransfer : MovementTransferOnJumpINfiniDy = MovementTransferOnJumpINfiniDy.PermaTransfer;
...
var movingPlatform : CharacterMotorMovingPlatformINfiniDy = CharacterMotorMovingPlatformINfiniDy();

class CharacterMotorSlidingINfiniDy {
...
var sliding : CharacterMotorSlidingINfiniDy = CharacterMotorSlidingINfiniDy();
...
private function UpdateFunction () {
	...
	if (grounded && !IsGroundedTest()) {
		...
		if (movingPlatform.enabled &&
			(movingPlatform.movementTransfer == MovementTransferOnJumpINfiniDy.InitTransfer ||
			movingPlatform.movementTransfer == MovementTransferOnJumpINfiniDy.PermaTransfer)
		) {
...
private function ApplyInputVelocityChange (velocity : Vector3) {	
	...
	if (movingPlatform.enabled && movingPlatform.movementTransfer == MovementTransferOnJumpINfiniDy.PermaTransfer) {
...
private function MoveWithPlatform () : boolean {
	return (
		...
		&& (grounded || movingPlatform.movementTransfer == MovementTransferOnJumpINfiniDy.PermaLocked)
